...
Гранула — это самодостаточное описание факта, извлечённого из диалога, кода или архитектурного решения. Гранула хранится в семантической памяти [selti](https://github.com/selti-project/selti) и может быть найдена по смысловому запросу.
...
8. Tool валидирует аргументы и отправляет гранулы в selti
...
selti автоматически дедуплицирует записи по `content_hash`. Если гранула с похожим содержимым уже существует, будет возвращена существующая запись.
...
granulate-tool.ts
    ...
    |  батчированная отправка в selti через MCP
    v
selti (PostgreSQL + pgvector)
```
